...
// v0.5.1 additions:
//
//   — Gaming component isolation:
//     configure({ allowSimulationComponents: false }) (default).
//     Components tagged 'gaming' or 'simulation' are rejected
//     at attachComponent() unless explicitly allowed.
//     loadCartridge() applies the same filter.
//     Hard boundary — not a comment.
//
//   — saveState() and loadState():
//     Serialise the current map, input, and component state
//     to a JSON string. Restore from a saved state string.
//     Enables persistent maps across sessions without
//     changing the execution model.
//
//   — Deployment config:
//     SorterSpine.configure(options) sets deployment-level
//     options. Currently:
//       allowSimulationComponents: bool (default false)
//       deploymentId: string (optional — for audit logging)
//       highStakes: bool (default false — gates confidence output)
//
//   — Spine header reference updated to sorter-runtime-v0_3_0.
//
// All v0.5.0 functionality preserved. Backwards compatible.
// ══════════════════════════════════════════════════════════
